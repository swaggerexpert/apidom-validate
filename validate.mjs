...
const definitionFileContent = fs.readFileSync(definitionFilePath, {
  encoding: "utf8",
  flag: "r",
});
...
(async () => {
  ...
  const warnings = validationResult.filter(
    (diagnostic) => diagnostic.severity === DiagnosticSeverity.Warning,
  );
  const information = validationResult.filter(
    (diagnostic) => diagnostic.severity === DiagnosticSeverity.Information,
  );
  const hints = validationResult.filter(
